...
import qualified Stage2.Check.LocalBinding as Local (Constraint (..), LocalBinding (..))
import qualified Stage2.Check.Mask as Mask
import qualified Stage2.Check.Simple.Data as Simple.Data
import qualified Stage2.Check.Simple.Evidence as Simple.Evidence (lift)
import qualified Stage2.Check.Simple.Type as Simple (instanciate, lift)
...
import qualified Stage2.Index.Evidence as Evidence (Index (..))
...
import qualified Stage2.Unify.Builtin as Builtin (constrain)
import Stage2.Unify.Class
  ( Collected (..),
    Collector (..),
    Functor (..),
    Generalizable (..),
...
import {-# SOURCE #-} Stage2.Unify.Error (Error (..), abort)
import Stage2.Unify.Evidence (Evidence)
import qualified Stage2.Unify.Evidence as Evidence (Box (..), Evidence (..), unify, unshift)
              ...
              Unsolved {kind = kind2, constraints = constraints2, erasure = erasure2}
                | constraints1 <- fmap pure constraints1,
                  ...
                  let merge left right = do
                        ...
                        pure left =
                    do
                      Stage2.Unify.Type.unify context position kind1 kind2
...
-- todo merge this with Stage2.Check.Temporary.Type checking somehow
